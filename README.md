...
- UI русифицирован на уровне отображения (подписи, кнопки, служебные тексты и названия статусов в интерфейсе).

## Как пользоваться панелью диспетчера
1. Войти через `/login` пользователем с ролью dispatcher.
2. Открыть `/dispatcher`.
3. При необходимости отфильтровать список заявок по статусу (`Новая`, `Назначена`, `В работе`, `Выполнена`, `Отменена`).
4. Для заявки в статусе `Новая`:
   - выбрать мастера из выпадающего списка;
   - нажать «Назначить».
5. Для заявки в статусе `Новая` или `Назначена`:
   - нажать «Отменить».
...
- Роли в интерфейсе отображаются на русском языке.
- Для статусов `in_progress`, `done`, `canceled` в панели диспетчера показывается пояснение о недоступности действий без добавления новых операций.

## Демо-данные заявок после seed
После `php artisan migrate --seed` в базе есть демонстрационные заявки с разными статусами для ручной проверки интерфейса:
- `new`
- `assigned`
- `in_progress`
- `done`
